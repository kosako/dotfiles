...
この repository は `~/dotfiles` に置く前提で運用している(下の手順の `--source ~/dotfiles` もこれに合わせている)。dotfiles 本体は「ディレクトリ規約」が判定対象にする開発 project ではなく home を管理する道具なので、置き場所は `~/dotfiles` を正本とする。

`~/src/<context>/` 規約に実体も揃えたい場合は、実体を `~/src/` 配下(例: `~/src/personal/dotfiles`)に置いて `~/dotfiles` から symlink を張る(下の手順をそのまま使える)か、`chezmoi --source <実パス>` で実体のパスを直接指定する。
