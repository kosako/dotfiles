...
- GitHub 由来の write / secret deny は専用 capability を作らず **`enforceAiSandbox` に
  相乗り**する(#119)。
- **既定は全 profile で false**(配管のみ・opt-in)。render は byte-identical で、実効化は
  別ステップ(Phase 2、検証ゲート付き)。
