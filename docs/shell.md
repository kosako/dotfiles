...
managed file はこれらを末尾で source するので、local 側が最終的に勝つ。例えばツールが `~/.zshrc` に自動追記する absolute path(`/Users/<you>/...`)は、managed file ではなく `~/.zshrc.local` に置く。

## 移行手順

既存の `~/.zshrc` / `~/.zprofile` があるため、apply は backup と diff 確認を伴って行う(この module 単体では apply しない。実際の apply は別途)。

1. **machine 固有行を退避**: 既存 `~/.zshrc` の中で managed に含めない行(ツール / IDE の PATH、tool env、secret)を `~/.zshrc.local` に移す。`~/.zprofile` も同様に `~/.zprofile.local` へ。
2. **backup**: `cp ~/.zshrc ~/.zshrc.pre-chezmoi`(`~/.zprofile` も同様)。
