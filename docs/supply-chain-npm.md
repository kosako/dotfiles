...
- npm は `min-release-age` を内部で `before`(= 現在 − 指定日数の日付)へ変換し、元のキーを削除する。このため `npm config get min-release-age` は設定済みでも常に `null` を返す。`doctor.sh` は実効キー `before` が設定されているかで honored を判定する(`min-release-age` の直読みでは検証できない)。
