...
PostToolUse + Stop。各 capability が自分の event だけを足し、両方 false で `hooks` キーが消えること。#137 / #199)を exact に確認する。
...
Claude 側と同じ exact pin(PreToolUse は timeout 10、PostToolUse / Stop は timeout なし。#181 / #199)に加え、
top-level key が `{hooks}` だけであること(Codex 0.142.5 の parse 制約 #185)、両 hook capability が false のとき
